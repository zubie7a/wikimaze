...
// Delegates to the active scene controller's getEffectiveSize() method
...
async function createMaze(wallData, customStartCell = null) {
    ...
    // Helper to check if a wall has a door (openspace, pillars, complex, and maze modes)
    const isDoorWall = (type, x, y) => {
        // Check maze scene doors
        if (sceneMode === 'maze' && window.mazeDoors) {
            const doors = window.mazeDoors;
            // North door: horizontal at y=0
            if (type === 'horizontal' && y === 0 && x === doors.north.x) return true;
            // South door: horizontal at y=SIZE
            if (type === 'horizontal' && y === SIZE && x === doors.south.x) return true;
            // West door: vertical at x=0
            if (type === 'vertical' && x === 0 && y === doors.west.y) return true;
            // East door: vertical at x=SIZE
            if (type === 'vertical' && x === SIZE && y === doors.east.y) return true;
            return false;
        }
        if (sceneMode !== 'openspace' && sceneMode !== 'pillars' && sceneMode !== 'complex') return false;
...
async function regenerateScene() {
    ...
    try {
        ...
        console.log(`[RegenerateScene] Mode: ${sceneMode}, Size: ${SIZE}, ActiveScene: ${activeScene?.id}`);
        ...
        // ALWAYS spawn at a door for all scenes (regardless of randomSceneChange)
        if (sceneMode === 'openspace' || sceneMode === 'pillars' || sceneMode === 'complex') {
            console.log(`[RegenerateScene] Spawning at door for ${sceneMode}`);
            // Spawn at a random door (layout is known or standard)
        ...
        // For maze scene, update spawn position AFTER generateMaze but BEFORE createMaze
        // (since doors are set during generateLayout and pathfinding needs correct spawn)
        if (sceneMode === 'maze' && window.mazeDoors) {
            const doors = window.mazeDoors;
            const doorOffset = 1.5;
            const doorKeys = ['north', 'south', 'east', 'west'];
            const randomKey = doorKeys[Math.floor(Math.random() * doorKeys.length)];
            const doorDef = doors[randomKey];
        ...
        // NOW calculate start cell for image loading order (after position is set)
        ...
        if (sceneMode === 'gallery' && window.galleryDoors) {
            ...
            console.log(`[RegenerateScene] Spawning at cathedral door with doors:`, window.cathedralDoors);
        ...
        if (sceneMode === 'cathedral') {
            ...
            if (activeScene && typeof activeScene.loadCathedralPaintings === 'function') {
                // Always call reloadAllPaintings - it handles cancellation properly
...
function init() {
    ...
    if (sceneMode === 'alley' || sceneMode === 'openspace' || sceneMode === 'maze') {
        scene.background = new THREE.Color(0x000000); // Dark for alley, openspace, and maze
        if (sceneMode === 'alley') {
        ...
        scene.background = new THREE.Color(0x87CEEB); // Sky blue for other modes
        scene.fog = null;
    }
    ...
    // Set initial player position based on mode (BEFORE createMaze for maze mode pathfinding)
    // ALWAYS spawn at a door for all scenes
    if (sceneMode === 'openspace' || sceneMode === 'pillars' || sceneMode === 'complex') {
        const SIZE = getEffectiveSize();
        console.log(`[Init] Spawning at door for ${sceneMode}, Size: ${SIZE}`);
        const newHalfSize = (SIZE * CELL_SIZE) / 2;
        const doorOffset = 1.5;
        const doors = ['north', 'south', 'east', 'west'];
        const randomDoor = doors[Math.floor(Math.random() * doors.length)];
    ...
    } else if (sceneMode === 'maze' && window.mazeDoors) {
        // Maze mode: spawn at a random door
        const SIZE = getEffectiveSize();
        const doors = window.mazeDoors;
        const doorOffset = 1.5;
        const doorKeys = ['north', 'south', 'east', 'west'];
        ...
        if (randomKey === 'north') {
            playerPosition.x = (doorDef.x - SIZE / 2) * CELL_SIZE + CELL_SIZE / 2;
            playerPosition.z = (-SIZE / 2) * CELL_SIZE + doorOffset;
            playerRotation = Math.PI; // Face south (into maze)
        } else if (randomKey === 'south') {
            playerPosition.x = (doorDef.x - SIZE / 2) * CELL_SIZE + CELL_SIZE / 2;
            playerPosition.z = (SIZE / 2) * CELL_SIZE - doorOffset;
            playerRotation = 0; // Face north (into maze)
        } else if (randomKey === 'west') {
            playerPosition.x = (-SIZE / 2) * CELL_SIZE + doorOffset;
            playerPosition.z = (doorDef.y - SIZE / 2) * CELL_SIZE + CELL_SIZE / 2;
            playerRotation = -Math.PI / 2;
    ...
    // Calculate start cell for pathfinding based on player spawn position
    const startCell = worldToGrid(playerPosition.x, playerPosition.z);
...
function isValidPosition(x, z) {
    ...
    if (sceneMode === 'alley') {
        ...
        const doors = window.complexDoors;
        if (doors) {
            const inNorthDoor = z < -halfSize + checkDist && x >= doors.north.minX && x <= doors.north.maxX;
            const inSouthDoor = z > halfSize - checkDist && x >= doors.south.minX && x <= doors.south.maxX;
            const inWestDoor = x < -halfSize + checkDist && z >= doors.west.minZ && z <= doors.west.maxZ;
            ...
            // Fallback
            if (x < -halfSize + checkDist || x > halfSize - checkDist || z < -halfSize + checkDist || z > halfSize - checkDist) return false;
        }
        ...
        return true;
    } else if (sceneMode === 'maze') {
        // Maze mode: allow crossing through doors at boundaries
        const doors = window.mazeDoors;
        if (doors) {
            const doorHalfWidth = doors.doorWidth / 2;

            // Calculate door world positions
            const northDoorX = (doors.north.x - SIZE / 2) * CELL_SIZE + CELL_SIZE / 2;
            const southDoorX = (doors.south.x - SIZE / 2) * CELL_SIZE + CELL_SIZE / 2;
            const westDoorZ = (doors.west.y - SIZE / 2) * CELL_SIZE + CELL_SIZE / 2;
            const eastDoorZ = (doors.east.y - SIZE / 2) * CELL_SIZE + CELL_SIZE / 2;

            const inNorthDoor = z < -halfSize + checkDist && Math.abs(x - northDoorX) < doorHalfWidth;
            const inSouthDoor = z > halfSize - checkDist && Math.abs(x - southDoorX) < doorHalfWidth;
            const inWestDoor = x < -halfSize + checkDist && Math.abs(z - westDoorZ) < doorHalfWidth;
            const inEastDoor = x > halfSize - checkDist && Math.abs(z - eastDoorZ) < doorHalfWidth;
    ...
    // Helper to check if a wall is a door wall (no collision in openspace/pillars/complex/maze mode)
    const center = Math.floor(SIZE / 2);
    const isDoorWall = (type, wallX, wallY) => {
        ...
        if (sceneMode === 'maze' && window.mazeDoors) {
            const doors = window.mazeDoors;
            // North door: horizontal at y=0
            if (type === 'horizontal' && wallY === 0 && wallX === doors.north.x) return true;
            // South door: horizontal at y=SIZE
            if (type === 'horizontal' && wallY === SIZE && wallX === doors.south.x) return true;
            // West door: vertical at x=0
            if (type === 'vertical' && wallX === 0 && wallY === doors.west.y) return true;
            // East door: vertical at x=SIZE
            if (type === 'vertical' && wallX === SIZE && wallY === doors.east.y) return true;
            return false;
        }
        if (sceneMode !== 'openspace' && sceneMode !== 'pillars' && sceneMode !== 'complex') return false;
        // North door: horizontal at y=0, x=center
...
function updateMovement() {
    ...
    // Door crossing for complex mode
    if (sceneMode === 'complex' && window.complexDoors) {
        const halfSize = (getEffectiveSize() * CELL_SIZE) / 2;
        let exitDirection = null;
        if (playerPosition.x < -halfSize) exitDirection = 'west';
        else if (playerPosition.x > halfSize) exitDirection = 'east';
    ...
    if (sceneMode === 'maze' && window.mazeDoors) {
        ...
        if (exitDirection && !isTransitioningRoom && !window.justRegenerated) {
            console.log('Maze door detected:', exitDirection);
            handleMazeDoorCrossing(exitDirection);
        }
    }
    ...
    // Door crossing for cathedral mode
    if (sceneMode === 'cathedral' && window.cathedralDoors && !isTransitioningRoom && !window.justRegenerated) {
        const doors = window.cathedralDoors;
        const halfRoom = doors.roomWidth / 2;
        const playerRadius = 0.4;
        const doorHalfWidth = doors.doorWidth / 2;

        // Check if player is at room boundary
        const atNorthWall = playerPosition.z >= halfRoom - playerRadius - 0.1;
        const atSouthWall = playerPosition.z <= -halfRoom + playerRadius + 0.1;
        const atEastWall = playerPosition.x >= halfRoom - playerRadius - 0.1;
        const atWestWall = playerPosition.x <= -halfRoom + playerRadius + 0.1;

        // Check door 1
        const checkDoor = (door) => {
            // Calculate door center position
            const doorCenterX = door.x;
            const doorCenterZ = door.z;

            if (door.direction === 'north' && atNorthWall) {
                return Math.abs(playerPosition.x - doorCenterX) < doorHalfWidth + 0.1;
            } else if (door.direction === 'south' && atSouthWall) {
                return Math.abs(playerPosition.x - doorCenterX) < doorHalfWidth + 0.1;
            } else if (door.direction === 'east' && atEastWall) {
                return Math.abs(playerPosition.z - doorCenterZ) < doorHalfWidth + 0.1;
            } else if (door.direction === 'west' && atWestWall) {
                return Math.abs(playerPosition.z - doorCenterZ) < doorHalfWidth + 0.1;
            }
            return false;
        };

        if (checkDoor(doors.door1)) {
            handleCathedralDoorCrossing(1);
        } else if (checkDoor(doors.door2)) {
            handleCathedralDoorCrossing(2);
        }
    }
...
async function handleAlleyCrossing() {
    ...
    if (randomSceneChange) {
        ...
        const availableScenes = ['gallery', 'alley', 'openspace', 'cathedral', 'pillars', 'complex'];
...
// Handle crossing maze door - regenerate maze and spawn at a random door
async function handleMazeDoorCrossing(exitDirection) {
    if (sceneMode !== 'maze') return;
    if (!collisionsEnabled) return;

    if (isTransitioningRoom) {
        console.log('Already transitioning maze, ignoring duplicate call');
        return;
    }
    isTransitioningRoom = true;
    ...
    try {
        ...
        if (randomSceneChange) {
            ...
            // Regenerate with new scene (this handles all spawn positioning)
            await regenerateScene();
        ...
        // Pick spawn door (different from exit)
        const doors = window.mazeDoors;
        if (doors) {
            const doorOffset = 1.5;
            const doorKeys = ['north', 'south', 'east', 'west'].filter(k => k !== exitDirection);
            const randomKey = doorKeys[Math.floor(Math.random() * doorKeys.length)];
            const doorDef = doors[randomKey];
...
async function handleOpenspaceDoorCrossing(exitDirection) {
    ...
    try {
        ...
        if (randomSceneChange) {
            ...
            // Regenerate with new scene (this handles spawn positioning including window.galleryDoors etc)
            await regenerateScene();
        ...
        // Non-random scene change: Stay in Openspace but resize and spawn at opposite door
        ...
        // Regenerate the entire scene with new size (openspace size is randomized within getEffectiveSize/regenerateScene or we need to force it?)
        // Actually generateMaze calls getEffectiveSize() which checks getRandomOpenspaceSize if implementation allows, but here we likely rely on randomness inside generateMaze for openspace?
        // Wait, maze.js doesn't store openspaceSize globally for generateMaze...
        // Ah, generateMaze(SIZE) uses the argument. regenerateScene calls generateMaze(getEffectiveSize()).
        // We should ensure the new size is used.
        // Actually getEffectiveSize() returns global effective size.
        // Let's assume regenerateScene handles it or just proceed.

        await regenerateScene();

        // Position player at the opposite door of the new room (Override random spawn from regenerateScene)
        const doors = window.openspaceDoors;
        if (doors) {
            const newHalfSize = doors.halfSize;
            const doorOffset = 1.5;
...
async function handlePillarsDoorCrossing(exitDirection) {
    ...
    try {
        ...
        if (randomSceneChange) {
            ...
            } else if (newSceneMode === 'complex') {
                const complexSize = getEffectiveSize();
                const complexHalfSize = (complexSize * CELL_SIZE) / 2;
                const doorOffset = 1.5;
                switch (exitDirection) {
                    case 'east':
                        playerPosition.x = -complexHalfSize + doorOffset;
                        playerPosition.z = 0;
                        playerRotation = -Math.PI / 2;
                        break;
                    case 'west':
                    ...
                    case 'south':
                        playerPosition.x = 0;
                        playerPosition.z = -complexHalfSize + doorOffset;
                        playerRotation = Math.PI;
                        break;
                    case 'north':
                        playerPosition.x = 0;
...
async function handleCathedralDoorCrossing(exitDoorIndex) {
    ...
    // Prevent concurrent door crossings
    if (isTransitioningRoom) {
        console.log('Already transitioning cathedral, ignoring duplicate call');
        return;
    }
    isTransitioningRoom = true;
    ...
    try {
        ...
        if (randomSceneChange) {
            ...
            if (newSceneMode === 'openspace') {
                ...
                // Spawn at opposite door in new cathedral
                const doors = window.cathedralDoors;
                if (doors) {
                    const spawnDoor = exitDoorIndex === 1 ? doors.door2 : doors.door1;
                    const doorOffset = 2.0;
                    // Position inside door based on direction
                    if (spawnDoor.direction === 'north') {
                        playerPosition.x = spawnDoor.x;
                        playerPosition.z = spawnDoor.z - doorOffset;
                        playerRotation = 0;
        ...
        console.log(`Crossed cathedral door ${exitDoorIndex}, regenerating cathedral...`);

        // Regenerate the cathedral scene
        await regenerateScene();
        ...
        if (doors) {
            ...
            if (spawnDoor.direction === 'north') {
                playerPosition.x = spawnDoor.x;
                playerPosition.z = spawnDoor.z - doorOffset;
                playerRotation = 0; // Face south (into room)
            } else if (spawnDoor.direction === 'south') {
                playerPosition.x = spawnDoor.x;
                playerPosition.z = spawnDoor.z + doorOffset;
                playerRotation = Math.PI; // Face north (into room)
            } else if (spawnDoor.direction === 'east') {
                playerPosition.x = spawnDoor.x - doorOffset;
                playerPosition.z = spawnDoor.z;
                playerRotation = Math.PI / 2; // Face west (into room)
            } else { // west
                playerPosition.x = spawnDoor.x + doorOffset;
                playerPosition.z = spawnDoor.z;
                playerRotation = -Math.PI / 2; // Face east (into room)
            }
        }
        ...
        statsDiv.innerHTML = `
            ...
            <div style="margin-bottom: 8px;">
                <select id="scene-mode-select" style="width: 100%; padding: 3px; background: #333; color: #fff; border: 1px solid #666;">
                    ...
                    <option value="complex" ${sceneMode === 'complex' ? 'selected' : ''}>Complex</option>
...
async function handleComplexDoorCrossing(exitDirection) {
    ...
    try {
        ...
        if (randomSceneChange) {
            ...
            // Position player
            if (newSceneMode === 'complex') {
                // Duplicate of complex logic below
                const size = getEffectiveSize();
                const halfSize = (size * CELL_SIZE) / 2;
                const doorOffset = 1.5;
                switch (exitDirection) {
